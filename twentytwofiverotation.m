%%%lossless 22.5° rotation MZ 2016
%   pixel grid
%   O   2   O
%        26.xx°
%   O   1   O
...
%   O   O   x
%
% + 1 required to fullfill nyquist
%  1: 45° -> length 1.4 -> requires sampling Y 1+1 = 2, sampling X -> 1 / 2%  
%  2: 26.xx°  -> length 2.4xxx -> sampling Y 2+1 = 3, sampling X -> 1 / 3
%  3: etc etc

    degree=atand(0.5/1); %22.5; % %
    
    hyp=1/(1/cosd(degree));

    tand(1/0.5)
    
    [x1,y1]=meshgrid(1:0.25:60,1:0.25:60);
    ...
    for i=1:length(x1(:))            
        dist(:)=sqrt(sum((repmat(coord(1:3,i),[1 length(x1(:))])-([x1(:),y1(:),zeros(length(x1(:)),1)])').^2,1));
        [idx,idy]=find(min(dist));
    end
